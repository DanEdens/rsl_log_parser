...
try:
    ...
    client.connect(os.environ.get("AWSIP"), os.environ.get("AWSPORT"))
except: pass
...
def main(x):
    ...
    try:
        client.publish('status/raid/energy', myvars['Energy'][:5], retain=True)
        client.publish('status/raid/silver', myvars['Silver'], retain=True)
        client.publish('status/raid/gems', myvars['Gem'], retain=True)
    except: pass
    print('Energy: ' + myvars['Energy'][:5])
